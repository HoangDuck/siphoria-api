...
func (hotelReceiver *HotelRepoImpl) GetListCheckInByHotel(context echo.Context, queryModel *query.DataQueryModel) ([]res.PaymentCheckInStatistic, int, int, error) {
	var listPayment []res.PaymentCheckInStatistic
	...
	err = err.Select("id", "room_type_id", "start_at", "user_id").
...
func (hotelReceiver *HotelRepoImpl) GetListHotelSearch(context echo.Context) ([]model.HotelSearch, error) {
	...
	value, errValue := strconv.ParseFloat(context.QueryParam("min_price"), 32)
	if errValue != nil {
		value = 0.0
	}
	min := float32(value)
	value, errValue = strconv.ParseFloat(context.QueryParam("max_price"), 32)
	if errValue != nil {
		value = math.MaxInt64
	}
	max := float32(value)
...
func (hotelReceiver *HotelRepoImpl) GetHotelFilter(queryModel *query.DataQueryModel) ([]model.Hotel, error) {
	...
	err = err.Where("(id in (Select hotel_id from hotel_works where user_id = ?) OR hotelier_id = ?)", queryModel.UserId, queryModel.UserId)
	var countTotalRows int64
	err.Model(model.Hotel{}).Count(&countTotalRows)
	queryModel.TotalRows = int(countTotalRows)
	err = err.Find(&listHotel)
...
func (hotelReceiver *HotelRepoImpl) CreateRequestPayout(payoutRequest model.PayoutRequest, paymentIds []string) (model.PayoutRequest, error) {
	...
	err := hotelReceiver.sql.Db.Raw("select * from fn_calculateTotalPricePayment(?,?)", payoutRequest.HotelId, strings.Join(paymentIds, ",")).Scan(&result)
